class Evaluator {
    ...
    methodCaller(callee, method, args) {
        if(callee != null) {
            switch (method) {
                ...
                case 'asTime': return this.m_asTime(callee);
            ...
            switch (method) {
                ...
                case 'anySet': return this.f_anySet(args[0], args[1], args[2], args[3], args[4]);
                default:
                    console.warn(`Ignoring call of "${method}" on undefined variable`);
            }
            return null;
        }
    }
    ...
    m_asDate(val) {
        ...
        if (!Evaluator._isDateValid(date)) {
            return '';
        }
        return new Date(val).toLocaleDateString(this.locale, {});
    }
    ...
    m_asTime(val) {
        ...
        return new Date(val).toLocaleTimeString(this.locale, {});
    }

    static _isDateValid(date) {
        return date.getTime() === date.getTime(); // false on NaN === NaN
    }
    ...
    f_anySet(val, result, prefix, suffix, otherwise) {
        //TODO: check if any subvalue in val is set
        //TODO: if do
    }
}